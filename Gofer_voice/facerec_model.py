    ...
    def run(self):
        ...
        while True:
            ...
            try:
                ...
                noface=noface+1
                if noface==5:
                    speech.say("Hello user, I can't see your face")
                    #subprocess.Popen(['python','speakme.py',"no face found"])
                    noface=0
            ...
            if name==name1:
                ...
                count=count+1
            if inc==10:
                facename = "hi "+str(name)
                speech.say(facename)
                #subprocess.Popen(['python','speakme.py',facename])
                name=" "
                name1=" "
                inc=0
                count=0
            ...
            if count == 20:
                ...
                #subprocess.Popen(['python','speakme.py',"Hi User. Please tell your name"])
                count=0
                cv2.destroyAllWindows()
                break
            name1=name
...
def startapp(mod, camid, filename):
    App(model=mod,
    camera_id=camid,
    cascade_filename=filename).run()
...
def facerec():        
    ...
    return (model,options.camera_id,options.cascade_filename)
    '''App(model=model,
