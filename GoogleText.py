...
def price_search(GS):
    ...
    try:
        for url in search(GS_A, start=0,num=3,stop=3):
            ...
            res = 'the price of ' + name +' in amazon is ' + str(price) + ' and is ' + str(avail) + '.'
            print res
            speech.say(res)
            cat = 'the category of the product is: ' + str(category)
            print(cat)
            speech.say(cat)
            pf = 1
            break
        if pf==0:
            print 'price is not found or unavailable in amazon'
            speech.say('price is not found or unavailable in amazon')
    except:
        print 'connection error occurred'
        speech.say('connection error occurred')
        return -1
    return pf
